...
function registrar_Cliente(array $datos, $con){
...
function registra_Usuario($datos, $con){
...
function usuario_existe($usuario, $con){
...
function email_existe($email, $con){
...
function login($usuario, $password, $con, $proceso)
{
    ...
    $sql = $con->prepare("SELECT id, usuario, password, id_cliente FROM usuarios WHERE usuario
     LIKE ? LIMIT 1");
    $sql->execute([$usuario]);
